...
foreach i $mwlib_types {
   foreach j $sub_mwlib_type { 
      lappend mw_lib [lindex [glob -type d $lib_dir/$i/$j ] 0 ] 
   }
}

# Form the Tech File and TLUplus parasitic information pointers
set tf_dir $lib_dir/tech/milkyway
set tlu_dir $lib_dir/tech/star_rc/
set_tlu_plus_files  -max_tluplus $tlu_dir/max/saed14nm_1p9m_Cmax.tluplus  \
                    -min_tluplus $tlu_dir/min/saed14nm_1p9m_Cmin.tluplus  \
                    -tech2itf_map  $tlu_dir/saed14nm_tf_itf_tluplus.map

# And create the Milkyway library for our design database storage
create_mw_lib ${top_design}.mw -technology $tf_dir/saed14nm_1p9m_mw.tf  -mw_reference_library $mw_lib -open
